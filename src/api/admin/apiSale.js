...
const getSalePage = async (req, res) => {
  try {
    ...
    let product = await axios.get(
      process.env.BASE_URL + `getSale/page/${page}`
    );
    let productCount = await axios.get(process.env.BASE_URL + `getSale`);
...
const addProductSale = async (req, res) => {
  ...
  let prodouct_id = req.body.productId;
  ...
  if (discount == "") {
    ...
    let dataProducts = await axios.put(
      process.env.BASE_URL + `products/sale/${prodouct_id}`,
      {
        sale: discount,
      }
    );
    ...
    if (dataProducts.data.statusCode == 200) {
      ...
      return res.render("success.ejs", {
        message: `Thêm sale thất bại ${dataProducts.data.message}`,
        url: referer,
      });
    }
  }
...
const updateDiscount = async (req, res) => {
  try {
    console.log(req.body);
    let dataProducts = await axios.put(
      process.env.BASE_URL + `products/sale/${req.body.productId}`,
      {
